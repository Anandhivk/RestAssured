		...
		for (int j = 1; j <=pagesize; j++) {
			...
			{    
				try {
					...
					for (Ingredients.PcosEliminate eliminate : Ingredients.PcosEliminate.pcosvalues()) {
					...
					// Allergy
					for (Ingredients.Allergies allergy : Ingredients.Allergies.PcosAllergyValue()) {
						if(ingredients.toLowerCase().contains(allergy.PcosAllergyValue().toLowerCase())) {
							System.out.println("Skip Recipe : "+recipeName +" Found Allergy: "+allergy.PcosAllergyValue());
							isAllergy=true;
							break;
						}
					}
